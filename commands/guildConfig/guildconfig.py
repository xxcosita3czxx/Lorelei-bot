...
class GuildConfig(commands.Cog):
    ...
    class configure_appear(app_commands.Group):
        ...
        @app_commands.command(name="color", description="Changes default color of embeds.")
        ...
        async def config_color_guild(self, interaction: discord.Interaction, color: str):
            try:
                ...
                await interaction.response.send_message(content=f"Set value {str(color)}", ephemeral=True)
            except Exception as e:
                await interaction.response.send_message(content=f"Exception happened: {e}", ephemeral=True)

        @app_commands.command(name="language", description="Set server default language")
        ...
        async def config_lang_guild(self, interaction: discord.Interaction, language: str):
            try:
                ...
                await interaction.response.send_message(content=f"Set value {str(language)}", ephemeral=True)
            except Exception as e:
                await interaction.response.send_message(content=f"Exception happened: {e}", ephemeral=True)
    ...
    class configure_ticketing(app_commands.Group):
        ...
        @app_commands.command(name="reviews", description="Review system")
        async def conf_ticketing_reviews(self, interaction: discord.Interaction, channel: discord.TextChannel = None, value: bool = None):
            try:
                ...
                if channel is not None and value is not None:
                    ...
                    response_message = response_template.format(values=f"{value}, {channel}") if response_template else "Value set"
                    ...
                    gconfig.set(interaction.guild_id, "Ticketing", "reviews-enabled", value=value)
                    response_message = response_template.format(values=f"{value}") if response_template else "Value set"
                    await interaction.response.send_message(content=response_message, ephemeral=True)

                elif channel is not None and value is None:
                    gconfig.set(interaction.guild_id, "Ticketing", "reviews-channel", value=channel)
                    response_message = response_template.format(values=f"{channel}") if response_template else "Value set"
                    await interaction.response.send_message(content=response_message, ephemeral=True)

                else:
                    await interaction.response.send_message(content="You have to choose", ephemeral=True)
    ...
    class configure_members(app_commands.Group):
        ...
        @app_commands.command(name="auto-role", description="Automatic role on join")
        @app_commands.describe(role="Role to add on join", enabled="Should it be enabled?")
        async def autorole(self, interaction: discord.Interaction, enabled: bool, role: discord.Role = None):
            try:
                ...
                await interaction.response.send_message(content=f"Set value {str(role.name)}, {str(enabled)}", ephemeral=True)
            except Exception as e:
                await interaction.response.send_message(content=f"Exception happened: {e}", ephemeral=True)
    ...
    class configure(app_commands.Group):
        ...
        @app_commands.command(name="reset", description="Resets the config. NO TAKIES BACKSIES, AS IT GETS DELETED PERMANENTLY")
        async def reset_config(self, interaction: discord.Interaction):
            try:
                os.remove(f"data/guilds/{interaction.guild.id}.toml")
                await interaction.response.send_message(content="Config Reset!", ephemeral=True)
            except FileNotFoundError:
                await interaction.response.send_message(content="No config generated yet! Try configuring the server", ephemeral=True)
            except PermissionError:
                await interaction.response.send_message(content="Permission Error! Ensure I have permissions for the file. If you're an administrator using Lorelei-bot, report this to Cosita Development!", ephemeral=True)
